...
Skill 会用计数工具持续记录实质动作与宿主报告的 context 用量。到期后先把事实、合同、测试和失败证据保存到各自权威文件，再重写短摘要，并校验摘要中的下一步与 `STATE.json` 完全一致。宿主支持可调用的原生压缩时才会执行压缩；否则只建立安全检查点，不会虚假声称当前会话 token 已减少。
...
V0.6 已加入上下文预算、可验证的工作摘要、检查点历史和候选长期记忆。状态 schema V5 继续兼容 V1-V4；brownfield 接管、UI、数据可视化、上下文启用、门禁顺序和旧状态兼容均有自动化测试。Skill 无法替代 Codex/Kimi 宿主执行其未开放的底层上下文压缩，也不会声称检查点本身降低了当前请求的 token 用量。
