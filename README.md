...
面向 Codex / Kimi Code 的两项配套能力：`prodloop` 负责企业级软件产品端到端交付，`codebase-audit` 负责对已有代码库进行轻量、只读、证据化的接管审计。`prodloop V0.5` 已内置企业业务 UI 与数据可视化设计、实现和验收门禁，不需要额外调用另一个 Skill。
...
V0.5 已加入企业数据可视化模块、指标与图表合同、独立数据和浏览器验收门禁。状态 schema V4 继续兼容 V1/V2/V3；brownfield 接管、门禁顺序、界面与可视化范围、旧状态非破坏性启用和证据约束均有自动化测试。尚未完成三个真实仓库前向测试，因此不声称已经证明能够高质量交付任意企业产品。
